from skimage.feature import local_binary_pattern, haar_like_feature, hog
import numpy as np
import cv2
...
import time
import os
from sklearn_extensions.extreme_learning_machines import elm, random_layer
...
def hog_feature(imagename):
    ...
    haar = hog(image, orientations=9, pixels_per_cell=(8,8), cells_per_block=(2,2), transform_sqrt=True, block_norm="L1")

    return haar  # the characteristic vertex is an array holding 256 * block ** 2 elements
...
def ELM_model():
    srhl_sigmoid = random_layer.MLPRandomLayer(n_hidden=1000, activation_func="sigmoid")
    return elm.GenELMClassifier(hidden_layer=srhl_sigmoid)
...
if __name__ == '__main__':
    ...
    for time_of_test in range(10):
        ...
        # model = linear_model.LogisticRegression(C=3, intercept_scaling=3)
